...
import '../css/livetracking.css'; // Make sure this is imported
...
const LiveTracking = () => {
    ...
    useEffect(() => {
        if (!mapInstance.current) {
            ...
            L.control.zoom({ position: 'bottomleft' }).addTo(map); // Zoom control moved to left
        ...
        <Layout>
            <div className="dashboard-content">
                ...
                <div className="tracking-wrapper">
                    ...
                    {/* Toggle Button Positioned by CSS */}
                    <button className="map-toggle-btn live-toggle-pos" onClick={toggleMapMode}>
                    ...
                    <div className="tracking-stats-panel">
                        <div className="panel-header">
                            <h3><i className="fas fa-location-crosshairs" style={{color:'#3699ff'}}></i> Status</h3>
                            <div className={`status-indicator ${isOnline ? 'status-online' : 'status-offline'}`}>
                                <div className="blink-dot"></div> {isOnline ? 'ONLINE' : 'OFFLINE'}
                            </div>
                        </div>
