...
// --- ICONS ---
// We use the existing start-marker.png to ensure it loads correctly.
// You can replace this path with '/icon/car.png' if you add a car icon later.
...
const LiveTracking = () => {
    ...
    const [isSatelliteMode, setIsSatelliteMode] = useState(true); // Default to Satellite

    // Refs
    const mapInstance = useRef(null);
    const markerRef = useRef(null);
    const tileLayerRef = useRef(null); // To handle map switching
    ...
    useEffect(() => {
        if (!mapInstance.current) {
            ...
            // Vehicle Marker (Using the fixed icon)
    ...
    const toggleMapMode = () => {
        if (mapInstance.current && tileLayerRef.current) {
            mapInstance.current.removeLayer(tileLayerRef.current); // Remove old
            const newUrl = isSatelliteMode ? STREET_MAP : SATELLITE_MAP; // Switch URL
            // Add new layer
            tileLayerRef.current = L.tileLayer(newUrl, { maxZoom: 20 }).addTo(mapInstance.current);
            setIsSatelliteMode(!isSatelliteMode); // Update state
        }
    };
        ...
        const unsubscribe = onValue(trackingRef, (snapshot) => {
            if (snapshot.exists()) {
                ...
                if (data.latitude && data.longitude) {
                    ...
                    // Check Online Status (within last 2 mins)
