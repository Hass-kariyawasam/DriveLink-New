...
// --- MAP STYLES CHANGED ---
// Light: OpenStreetMap (Clean)
const LIGHT_MAP = 'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
// Dark: Google Satellite Hybrid (Roads are visible clearly)
...
const TripManage = () => {
    ...
    useEffect(() => {
        if (!mapInstance.current) {
            ...
            // Default to Google Satellite (Dark Mode feel but clear roads)
            tileLayerRef.current = L.tileLayer(DARK_MAP, {
                maxZoom: 20
            }).addTo(map);
        ...
        <Layout>
            <div className="dashboard-content">
                ...
                <div className="map-wrapper">
                    ...
                    <button className="map-toggle-btn" onClick={toggleMapTheme} title="Switch Mode">
                        <i className={`fas ${isDarkMode ? 'fa-satellite' : 'fa-map'}`}></i>
                    </button>

                    {/* NEW Professional Speedometer - Bottom Center */}
                    <div className="speed-hud-pro">
                        <div className="speed-circle">
                            <span className="speed-val">{stats.speed.toFixed(0)}</span>
                            <span className="speed-lbl">KM/H</span>
                        </div>
                    </div>
                ...
                <div className="trip-info-grid">
                    ...
                    <Card icon="fa-clock" label="Duration" value={`${Math.floor(stats.totalDuration)} min`} color="#8950fc" />
                    <Card icon="fa-rocket" label="Top Speed" value={`${stats.topSpeed.toFixed(0)} km/h`} color="#f64e60" />
                    ...
                    <Card icon="fa-route" label="Range" value={`${stats.range.toFixed(0)} km`} color="#00d2fc" />
                </div>

                <div className="trip-controls">
                    <div className="input-row">
                        <div className="input-group"><input type="text" placeholder="Trip Name (Auto)" value={tripName} onChange={(e) => setTripName(e.target.value)} /></div>
                    ...
                    <div className="control-buttons">
                        {!trackingActive ? 
                            <button className="btn-modern btn-start" onClick={handleStart}><i className="fas fa-play"></i> START TRIP</button> : 
                            <button className="btn-modern btn-end" onClick={handleEndClick}><i className="fas fa-stop"></i> END TRIP</button>
                        }
                    </div>
                </div>
