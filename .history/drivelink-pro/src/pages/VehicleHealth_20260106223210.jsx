...
const VehicleHealth = () => {
    ...
    const [config, setConfig] = useState({
        serviceInterval: 5000, // Default 5000km
        tankCapacity: 50,
        fuelPrice: 380,
        manualOdometer: 0
    });
    ...
    const [isConfigOpen, setIsConfigOpen] = useState(false); // Toggle Settings
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            if (user) {
                ...
                try {
                    ...
                    // Load Trip History for Aggregation
        ...
        return () => unsubscribe();
    }, [config.manualOdometer]); // Re-calc if manual odometer changes
    ...
    // Idle Score (Low idle is better)
    const idlePercent = data.totalDuration > 0 ? ((data.totalIdleTime / data.totalDuration) * 100).toFixed(0) : 0;

    // Overall Health Score Calculation
    let healthScore = 100;
    if (kmRemaining < 500) healthScore -= 20; // Service due soon
    if (data.batteryVolts < 12.2) healthScore -= 15; // Bad battery
    if (idlePercent > 30) healthScore -= 10; // High idling
    if (avgMpg < 8) healthScore -= 5; // Poor efficiency
        ...
        <Layout>
            <div className="dashboard-content">
                ...
                <div className="vh-container">
                    ...
                    {/* --- TOP HERO SECTION (Health & Service) --- */}
