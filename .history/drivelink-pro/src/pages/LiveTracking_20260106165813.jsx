...
const LiveTracking = () => {
    ...
    useEffect(() => {
        if (!mapInstance.current) {
            ...
            // Mobile Zoom Control Position fix
    ...
    return (
        <Layout>
            <div className="dashboard-content">
                {/* Desktop Header (Hidden on Mobile Fullscreen) */}
                <div className="main-header hide-on-mobile">
                ...
                <div className="tracking-wrapper">
                    ...
                    <div className="map-controls-group">
                        ...
                        {/* 1. Green Online Status */}
                        <div className={`status-badge ${isOnline ? 'online' : 'offline'}`}>
                            <div className="blink-dot"></div>
                            {isOnline ? 'ONLINE' : 'OFFLINE'}
                        </div>

                        {/* 2. Satellite Signal Bars */}
...
// Component to draw signal bars
const SignalBars = ({ count }) => {
    // 0-3: 1 bar, 4-6: 2 bars, 7-9: 3 bars, 10+: 4 bars
    let activeBars = 1;
    ...
    return (
        <div className="signal-bars-icon">
            {[1, 2, 3, 4].map(bar => (
                <div key={bar} 
                     className={`bar ${bar <= activeBars ? 'active' : ''}`}
                     style={{height: `${bar * 3 + 4}px`}} // Dynamic height
                ></div>
            ))}
        </div>
    );
