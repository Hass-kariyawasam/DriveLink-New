...
// 🔥 FIX 1: Missing SignalBars Component එක මෙතන හැදුවා
const SignalBars = ({ count }) => {
    // චන්ද්‍රිකා ගණන අනුව පාට වෙනස් වීම
    let color = '#f64e60'; // Red (Weak)
    if (count >= 4) color = '#ffa800'; // Orange (Medium)
    if (count >= 7) color = '#1bc5bd'; // Green (Strong)

    return (
        <div style={{ display: 'flex', gap: '2px', alignItems: 'flex-end', height: '12px' }}>
            <div style={{ width: '3px', height: '4px', background: count > 0 ? color : '#333' }}></div>
            <div style={{ width: '3px', height: '7px', background: count > 3 ? color : '#333' }}></div>
            <div style={{ width: '3px', height: '10px', background: count > 6 ? color : '#333' }}></div>
            <div style={{ width: '3px', height: '12px', background: count > 9 ? color : '#333' }}></div>
        </div>
    );
};
        ...
        <Layout>
            <div className="dashboard-content">
                ...
                <div className="trip-container"> 
                    ...
                    </div> {/* 🔥 FIX 2: Close map-wrapper div */}
                </div> {/* 🔥 FIX 2: Close trip-container div */}
