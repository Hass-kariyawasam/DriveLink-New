...
const Sidebar = () => {
  const [isExpanded, setIsExpanded] = useState(true); // Sidebar එක දිගහැරිලා තියෙන්න
  const [isReportsOpen, setIsReportsOpen] = useState(false); // Reports menu එක සඳහා
  const [username, setUsername] = useState("--");
  
  const location = useLocation();
  const navigate = useNavigate();

  // User Name එක Firebase වලින් ගන්න
  useEffect(() => {
    ...
    <nav id="sidebar" className={isExpanded ? 'expanded' : ''}>
      ...
      <div className="sidebar-header">
        <div className="logo-container">
          <img src="logo.png" alt="DriveLink" className="logo-img" /> {/* logo.png public folder එකේ තියෙන්න ඕන */}
          <span className="logo-text">
             <span className="highlight">Drive</span><span className="highlight2">link</span>
          </span>
        </div>
      ...
      <ul className="sidebar-menu">
        ...
        {/* Username Section (ඔයාගේ HTML එකේ තිබ්බ විදියටම) */}
        <li>
            <p className="username-text" id="usernametag" style={{marginLeft: isExpanded ? '20px' : '0', display: isExpanded ? 'block' : 'none'}}>
                Hi, {username}!
            </p>
        ...
        {/* Reports with Submenu (Dropdown) */}
        <li className={`has-submenu ${isReportsOpen ? 'active' : ''}`}>
          <a href="#" onClick={(e) => { e.preventDefault(); toggleReports(); }}>
            <i className="fa-solid fa-flask-vial"></i>
            <span>Reports</span>
            <i className={`fas fa-chevron-${isReportsOpen ? 'up' : 'down'} arrow`} style={{marginLeft:'auto'}}></i>
          </a>
          {/* Submenu List */}
          <ul className="sub-menu" style={{ display: isReportsOpen ? 'block' : 'none' }}>
